        ...
        <div class="container-fluid">
            <div class="bg-body-tertiary rounded p-5">
                <div class="col-sm-8 mx-auto">
                    <a href="{{ route('adbuah') }}" class="btn btn-primary mt-5">+Tambah Buah</a>
                    <table class="table table-hover mt-5">
                        <thead class="table-primary">
                            <tr>
                                <th scope="row">No</th>
                                <th scope="row">Nama Buah</th>
                        ...
                        <tbody>
                            @forelse ($data as $item)
                                <tr>
                                    ...
                                    <td scope="col">{{ $item->nama_buah }}</td>
                                    <td scope="col">{{ $item->stok_buah }}</td>
                                    <td scope="col">{{ $item->harga_buah }}</td>
                                    <td scope="col">
                                        <form action="{{ route('deletebuah', ['id' => $item->id]) }}" method="POST">
                                            <a href="{{ route('editbuah', ['id' => $item->id]) }}"
